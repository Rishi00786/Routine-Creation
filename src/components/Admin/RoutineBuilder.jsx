...
const RoutineBuilder = ({ data = {}, isEdit }) => {

  const { setYourRoutines, yourRoutines, setShowRoutineBuilder, isAdmin, id } = useStateContext()
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      // Set loading to false when the upload completes (success or failure)
      setLoading(false);
    }
  };
  ...
  const handleUpdateRoutine = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${api_url}/routines/update/${id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        console.error('Failed to update routine:', await response.text());
  ...
  return (
    <>
      <div className={`w-[90vw] lg:w-[60vw] h-auto max-h-[80vh] my-8 bg-gray-200 rounded-xl p-8 shadow-xl border border-black absolute z-10 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 overflow-y-auto ${loading && "hidden"}`}>
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-extrabold text-gray-700 mb-6 text-center main4">
            Routine Builder
          </h1>

          <div onClick={() => setShowRoutineBuilder(false)} className="text-2xl font-extrabold text-gray-700 mb-6 text-center main4">
            <ImCross />
          </div>
        </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-4">
              <button
                ...
                className={`text-white bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none shadow-lg shadow-purple-500/50 dark:shadow-lg dark:shadow-purple-800/60 font-medium rounded-lg text-lg px-5 py-2.5 text-center ${routineData.milestones === "Weekly" ? "ring-4 ring-purple-800 dark:ring-purple-800 border-2 border-purple-600" : ""
                  }`}
              >
                Weekly
              </button>
              <button
                ...
                className={`text-white bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none shadow-lg shadow-purple-500/50 dark:shadow-lg dark:shadow-purple-800/60 font-medium rounded-lg text-lg px-5 py-2.5 text-center ${routineData.milestones === "Bi-Weekly" ? "ring-4 ring-purple-800 dark:ring-purple-800 border-2 border-purple-600" : ""
                  }`}              >
                Bi-Weekly
              </button>
            </div>
            ...
            {routineData.steps.map((step, index) => (
              <div key={index} className="flex flex-col items-center justify-center gap-2">
                ...
                <div className="flex lg:flex-row flex-col items-center justify-center gap-4 w-full">
                  ...
                  <div className="flex flex-col items-start justify-center w-full lg:w-2/3">
                    <label className="block mb-2 text-gray-600 font-semibold main2 mt-4">
                      How to use it:
                    </label>
                    <input
        ...
        <div className="flex justify-between items-center mt-8">
          ...
          {step < 7 ? (
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              {isAdmin && isEdit && <button
                onClick={handleUpdateRoutine}
                className="flex main2 items-center justify-center shadow-lg gap-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600"
              >
                Save Changes
                <BsSave2Fill className="text-xl" />
              </button>}
            </div>
          )}
        </div>
